...
def visu(files):
    ...
    for f in tqdm(files_csv):
        ...
        timy = pd.date_range(start=start_time, end=end_time, periods=200)
        ...
        for i in range(window_size, len(timy)):
            end_time = timy[i]
            count_all_time = ((all_time >= start_time) & (all_time <= end_time)).sum()/len(files_csv)
            start_time = timy[i - window_size]
            count = ((add >= start_time) & (add <= end_time)).sum()
            count_size = ((add_tot >= start_time) & (add_tot <= end_time)).sum()
            counts.append(count/count_all_time)
            counts_size.append(count_size/count_all_time)
        ...
        df_final.loc[ou] = [f'{timy[0].day}/{timy[0].month}/{timy[0].year}',len(df[df['action'] == 'T']),np.var(df[df['action'] == 'T']['price'].to_numpy()),np.max(np.array(counts)),np.min(np.array(counts)),np.mean(np.array(counts_size)/np.array(counts)), np.min(np.array(intens)),np.mean(intens[:int(extreme*len(intens))]) ]
        ou+=1
