...
imb_tot = [imb_add,imb_cancel]
price_tot = [price_add,price_cancel]

print(f"A quel point c'est long: {len(imb_add)}")
...
def visu_event_vs_imbalance(imb_tot, price_tot, string, bound = 0.9):
    ...
    counts_total = np.array(counts_tot[0])+np.array(counts_tot[1])
    for i in range (len(imb_tot)):
...
# %%
visu_imbalance_respec(imb_tot, price_tot, 0 , 'add', 10000)
visu_imbalance_respec(imb_tot, price_tot, 1 , 'cancel', 15000)
visu_event_vs_imbalance(imb_tot, price_tot, ['Cancel',' Add'], bound = .95)
...
# %%
visu_intensity_respec(imb_tot, intensity_tot, 1 , 'add', 10000)
visu_intensity_respec(imb_tot, intensity_tot, 2 , 'cancel', 9000)
...
for f in tqdm(files_csv):
    ...
    sizes = np.unique(np.array((np.unique(df['bid_sz_01'].to_numpy())).tolist() + (np.unique(df['ask_sz_01'].to_numpy())).tolist()))
    sizes.sort()
    ...
    for row in df.itertuples():
        ...
        if row.side == 'A':
            taille = row.ask_sz_01
        elif row.side == 'B':
            taille = row.bid_sz_01
        if row.action == 'A':
...
intensities = filtrage(intensities, 30, threshold=10)
